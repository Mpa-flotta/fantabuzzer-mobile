...
function join(){
  const n = $("team").value.trim();
  if (!n) return;
  myTeam = n;
  localStorage.setItem("fantabuzzerTeam",n);
  socket.emit("team:join",{team:n});
...
document.querySelectorAll(".bid").forEach(b=>{
  b.onclick=()=>{
    if(navigator.vibrate) navigator.vibrate(25);
    socket.emit("team:bid",{amount:Number(b.dataset.inc)});
  };
});
...
socket.on("auction:closed",({playerName})=>toast(`${playerName}: nessuna offerta`));
...
socket.on("state",s=>{
  ...
  if(t){
    ...
    $("myCount").textContent=t.roster.length;
    $("roster").innerHTML=t.roster.length
      ? t.roster.map(p=>`<div class="rowItem"><div><b>${esc(p.name)}</b><small>${esc(p.role)} ${esc(p.club)}</small></div><strong>${p.price}</strong></div>`).join("")
...
function renderTimer(){
  ...
  if(!a?.running||!a.endsAt){$("timer").textContent="--";return;}
